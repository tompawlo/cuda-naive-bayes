...
void print_words_and_occurencies() {
	for (size_t i = 0; i < words.size(); i++) {
		//cout << words[i] << " " << spam_occurencies[i] << " " << nonspam_occurencies[i] << endl;
...
void create_words_vector() {
	...
	//cout << spam_length + nonspam_length << endl;
	//spam_occurencies = new int[10*(spam_length + nonspam_length)];
	//nonspam_occurencies = new int[10*(spam_length + nonspam_length)];
...
void run_cpu() {
	...
	for (int i = 0; i < words.size(); i++) {
		...
		//cout << "spam_vector" << spam_vector << endl;
		//cout << "nonspam_vector" << nonspam_vector << endl;
		//cout << "binary_vector = " << binary_vector[i] << endl;
		//cout << "P(spam|test) = " << p_test_spam << endl;
		//cout << "P(nonspam|test) = " << p_test_nonspam << endl;
	}
...
void run_gpu() {
	...
	//for (size_t i = 0; i < dim; i++) vector_binary[i] = binary_vector[i];
	//for (size_t i = 0; i < dim; i++) cout<<vector_binary[i];
	
	// NONSPAM_VECTOR

	// HOST ----> DEVICE
	if (cudaMemcpy(dev_array, nonspam_vector, double_array_dim, cudaMemcpyHostToDevice) != cudaSuccess) {
		cout << "Nie mozna bylo skopiowac! 1" << endl;
		return;
	}

	if (cudaMemcpy(dev_binary, binary_vector, bool_array_dim, cudaMemcpyHostToDevice) != cudaSuccess) {
		cout << "Nie mozna bylo skopiowac! 2" << endl;
		return;
	}

	// mnozymy wektor na gpu
	int blockSize = 1024;
	...
	if (cudaMemcpy(vector, dev_array, sizeof(double) * dim, cudaMemcpyDeviceToHost) != cudaSuccess) {
		cout << "Nie mozna bylo skopiowac! 3" << endl;
		return;
	}
	//wynik znajduje sie w pierwszym elemencie wektora
	...
	if (cudaMemcpy(dev_array, spam_vector, double_array_dim, cudaMemcpyHostToDevice) != cudaSuccess) {
		cout << "Nie mozna bylo skopiowac! 4" << endl;
		return;
	}


	// mnozmy wektor na gpu
	gpu_multiplication << <1, blockSize >> >(dev_array, dev_binary, dim);
	cudaDeviceSynchronize();

	// DEVICE ---> HOST
	if (cudaMemcpy(vector, dev_array, sizeof(double) * dim, cudaMemcpyDeviceToHost) != cudaSuccess) {
		cout << "Nie mozna bylo skopiowac! 5" << endl;
		return;
	}
	//wynik znajduje sie w pierwszym elemencie wektora
